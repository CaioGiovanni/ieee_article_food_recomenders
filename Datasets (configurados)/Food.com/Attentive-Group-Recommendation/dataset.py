...
class GDataset(object):
    ...
    def __init__(self, user_path, group_path, num_negatives):
        ...
        # user data
        self.user_trainMatrix = self.load_rating_file_as_matrix(user_path + "Train.txt", user_path + "Test.txt", user_path + "Negative.txt")
        ...
        # group data
        self.group_trainMatrix = self.load_rating_file_as_matrix(group_path + "Train.txt", group_path + "Test.txt", group_path + "Negative.txt")
    ...
    def load_rating_file_as_matrix(self, filename, filename_test, filename_negative):
        # Get number of users and items
        num_users, num_items = 0, 0
        with open(filename, "r") as f, open(filename_test, "r") as t, open(filename_negative, "r") as n:
            line = f.readline()
            while line != None and line != "":
                arr = line.split(" ")
            ...
            line = n.readline()
            while line != None and line != "":
                k = line.split(',')[1].replace(')', '').split(' ')
                i = [int(x) for x in k]
                u = int(line.split(',')[0].replace('(', ''))
                i.append(num_items)
                num_items = max(i)
                num_users = max(num_users, u)
                line = n.readline()
    ...
    def get_group_dataloader(self, batch_size):
        ...
        print(torch.max(group_train_loader.dataset.tensors[0]))
