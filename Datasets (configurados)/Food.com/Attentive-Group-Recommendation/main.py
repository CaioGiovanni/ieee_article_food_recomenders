...
def evaluation(model, helper, testRatings, testNegatives, K, type_m, epoch):
    model.eval()
    precision, hr, ndcg, f1_score, mae, rmse = helper.evaluate_model(model, testRatings, testNegatives, K, type_m, epoch)
    return precision, hr, ndcg, f1_score, mae, rmse
...
if __name__ == '__main__':
    ...
    for epoch in range(config.epoch):
        ...
        precision, hr, ndcg, f1_score, mae, rmse = evaluation(agree, helper, dataset.group_testRatings, dataset.group_testNegatives, config.topK, 'group', epoch)
        print(
            'Group Iteration {} [{:.1f} s]: Precision = {:.4f}, '
            'HR = {:.4f}, '
            'NDCG = {:.4f}, '
            'F1-Score = {:.4f}, '
            'MAE = {}, '
            'RMSE = {}, [{:.1f} s]'.format(epoch, time() - t1, precision, hr, ndcg, f1_score, mae, rmse, time() - t2))
