...
for line in lines:
    line_splited = line.split("	")
    if (int(line_splited[1]) + 100000000) in product_id_full and line_splited[3].split('|')[5] == '1':
...
for line in lines:
    ...
    if line_splited[4] == "Music\n":
        product_id_full.append(int(line_splited[0]) + 300000000)
...
for line in lines:
    line_splited = line.split("	")
    if (int(line_splited[1]) + 300000000) in product_id_full and line_splited[3].split('|')[5] == '1':
        group_full_new.append(int(line_splited[0]))
